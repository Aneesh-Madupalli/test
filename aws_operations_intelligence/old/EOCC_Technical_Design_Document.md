---
title: EOCC Technical Design Document
document: 3 of 3
...
# EOCC — Technical Design Document (TDD)

**Audience:** Engineers, architects, security  
**Requirements:** [EOCC_PRD.md](EOCC_PRD.md)
...
| Item | Decision |
|---|---|
| Compute | 3× ECS Fargate: frontend-ecs, backend-ecs, agents-ecs |
| API | FastAPI — auth, RBAC, REST |
| Agents | LangGraph — 4 gather + synthesis |
| Structured data | Aurora PostgreSQL |
| Semantic search | Cortex RAG → Pinecone (text-embedding-3-large) |
| LLM | Cortex gateway + LLM Router |
| Cache | Redis |
| Events | EventBridge + SQS |
| Phase 1 connectors | AWS, Splunk, Jira, Confluence, GitHub (read) |

---

## 2. Data Architecture (locked design)
...
## 3. Aurora PostgreSQL — Enterprise Schema Design
...
## 4. Original 20 Use Cases — Architecture Fulfillment Matrix
...
## 5. Parallel Agents — Simple Design (accuracy first)
...
## 6. All 20 Use Case Working Flows
...
## 7. Technical System Architecture (locked)
...
| POST | `/api/copilot/query` | NL query → router → agent(s) |
| POST | `/api/agents/investigate` | Start investigation |
| GET | `/api/investigations/{id}` | Investigation status |
| GET | `/api/investigation-package/{id}` | InvestigationPackage JSON |
| GET | `/api/services` | Service catalog |
| GET | `/api/services/{code}/ownership` | Ownership card |
| GET | `/api/services/{id}/dependencies` | Blast radius |
| GET | `/api/business-processes` | Process health |
| GET | `/api/admin/connectors` | Connector status (ROLE-SRE) |

Next.js never calls agents-ecs or Cortex directly.

**OpenAPI:** `openapi/eocc-v1.yaml` at implementation.

---

## 9. IAM and Security (conceptual)

| Service | Phase 1 access |
|---|---|
| frontend-ecs | No AWS SDK |
| backend-ecs | Aurora, SQS, S3, Secrets Manager, connector read APIs |
| agents-ecs | Aurora, Redis; Cortex via enterprise path only |

- Private subnets for all ECS tasks  
- agents-ecs internal only (Cloud Map from backend-ecs)  
- RBAC per PRD §8 (`ROLE-LEAD`, `ROLE-OPS`, `ROLE-SRE`, `ROLE-OWNER`, `ROLE-BUILDER`)

**IAM JSON templates:** `iam/` at implementation.

---

## 10. DDL Appendix

Phase 1 implements **16 tables** per schema Section 3. Full `CREATE TABLE` at implementation.
